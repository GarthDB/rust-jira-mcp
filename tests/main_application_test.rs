...
async fn test_main_application_metadata_creation() {
    ...
    startup_metadata.insert("api_url".to_string(), "https://test.example.com/rest/api/2".to_string());
    ...
    assert_eq!(shutdown_metadata.get("reason"), Some(&"normal_shutdown".to_string()));
}

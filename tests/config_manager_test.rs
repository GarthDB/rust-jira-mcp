...
async fn test_load_with_options_with_config_files() {
    ...
    assert_eq!(config.api_base_url, "https://local-jira.example.com/rest/api/2");
...
"#;
    ...
    let config = manager.get_config().await;
    assert_eq!(config.api_base_url, "https://yaml-jira.example.com/rest/api/2");
    assert_eq!(config.email, "yaml@example.com");
    assert_eq!(config.personal_access_token, "yaml-token");
    assert_eq!(config.default_project, Some("YAML".to_string()));
    assert_eq!(config.max_results, Some(600));
...
async fn test_get_config_sources_with_env() {
    ...
    assert!(sources.iter().any(|s| matches!(s, ConfigSource::Environment)));
...
async fn test_config_manager_clone() {
    ...
    assert_eq!(manager.is_hot_reload_enabled(), cloned.is_hot_reload_enabled());
    ...
    for (s1, s2) in sources1.iter().zip(sources2.iter()) {
        match (s1, s2) {
            (ConfigSource::Default, ConfigSource::Default) => {},
            (ConfigSource::Environment, ConfigSource::Environment) => {},
            (ConfigSource::DotEnv, ConfigSource::DotEnv) => {},
            (ConfigSource::Toml(p1), ConfigSource::Toml(p2)) => assert_eq!(p1, p2),
            (ConfigSource::Yaml(p1), ConfigSource::Yaml(p2)) => assert_eq!(p1, p2),
            (ConfigSource::Json(p1), ConfigSource::Json(p2)) => assert_eq!(p1, p2),
            _ => panic!("Source type mismatch"),
        }
    }
}

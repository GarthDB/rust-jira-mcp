...
fn test_config_validation_error_display() {
    ...
    assert_eq!(
        error.to_string(),
        "Invalid URL in field 'api_url': 'not-a-url'"
    );
    ...
    assert_eq!(
        error.to_string(),
        "Field 'timeout' value 500 is outside valid range [1, 300]"
    );
    ...
    assert_eq!(
        error.to_string(),
        "Invalid file path in field 'log_file': '/invalid/path'"
    );
    ...
    let error = ConfigValidationError::ConfigFileParseError(
        "config.toml".to_string(),
        "Invalid TOML".to_string(),
    );
    assert_eq!(
        error.to_string(),
        "Failed to parse configuration file 'config.toml': Invalid TOML"
    );
    ...
    assert_eq!(
        error.to_string(),
        "Required environment variable 'JIRA_TOKEN' is not set"
    );

    // Test InvalidEnvironmentVariable
    let error = ConfigValidationError::InvalidEnvironmentVariable(
        "JIRA_TIMEOUT".to_string(),
        "invalid".to_string(),
    );
    assert_eq!(
        error.to_string(),
        "Invalid value for environment variable 'JIRA_TIMEOUT': 'invalid'"
    );
}
...
fn test_config_validation_error_serialization() {
    ...
    let errors = vec![
        ...
        ConfigValidationError::ConfigFileParseError(
            "config.toml".to_string(),
            "Invalid TOML".to_string(),
        ),
        ConfigValidationError::MissingEnvironmentVariable("JIRA_TOKEN".to_string()),
        ConfigValidationError::InvalidEnvironmentVariable(
            "JIRA_TIMEOUT".to_string(),
            "invalid".to_string(),
        ),
    ];
    ...
    for error in errors {
        ...
        match (&error, &deserialized) {
            ...
            (
                ConfigValidationError::InvalidUrl(f1, u1),
                ConfigValidationError::InvalidUrl(f2, u2),
            ) => {
                assert_eq!(f1, f2);
                assert_eq!(u1, u2);
            }
            (
                ConfigValidationError::InvalidRange(f1, v1, min1, max1),
                ConfigValidationError::InvalidRange(f2, v2, min2, max2),
            ) => {
                assert_eq!(f1, f2);
                assert_eq!(v1, v2);
                assert_eq!(min1, min2);
            ...
            (
                ConfigValidationError::InvalidFilePath(f1, p1),
                ConfigValidationError::InvalidFilePath(f2, p2),
            ) => {
                assert_eq!(f1, f2);
                assert_eq!(p1, p2);
            }
            (
                ...
                ConfigValidationError::ConfigFileParseError(f2, e2),
            ) => {
                assert_eq!(f1, f2);
                assert_eq!(e1, e2);
            }
            (
                ConfigValidationError::MissingEnvironmentVariable(v1),
                ConfigValidationError::MissingEnvironmentVariable(v2),
            ) => {
                assert_eq!(v1, v2);
            }
            (
                ConfigValidationError::InvalidEnvironmentVariable(v1, val1),
                ConfigValidationError::InvalidEnvironmentVariable(v2, val2),
            ) => {
                assert_eq!(v1, v2);
                assert_eq!(val1, val2);
            }
...
fn test_validation_rule_custom_validator() {
    let rule = ValidationRule::new("test_field".to_string()).custom_validator(|value| {
        if value.len() > 10 {
...
fn test_config_validator_validate_custom_validator() {
    let rule = ValidationRule::new("username".to_string()).custom_validator(|value| {
        if value.len() < 3 {
...
fn test_matches_pattern() {
    ...
    let email_rule = ValidationRule::new("email".to_string()).custom_validator(|value| {
        if !value.contains('@') || !value.contains('.') {
            Err("Invalid email format".to_string())
        } else {
            Ok(())
    ...
    let url_rule = ValidationRule::new("url".to_string()).custom_validator(|value| {
...
fn test_config_validator_clone() {
    let validator =
        ConfigValidator::new().add_rule(ValidationRule::new("email".to_string()).required());

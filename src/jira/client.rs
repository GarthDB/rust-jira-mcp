...
use crate::types::jira::{JiraIssue, JiraSearchResult, JiraProject, JiraComment, JiraTransition};
...
impl JiraClient {
            ...
            match request_builder.send().await {
                Ok(response) => {
                    ...
                    if retry_count < max_retries && Self::should_retry(status) {
                        ...
                        warn!("Retrying request in {:?} (attempt {}/{})", delay, retry_count, max_retries);
                ...
                Err(e) => {
                    ...
                    if retry_count < max_retries && e.is_timeout() {
                        ...
                        // retry_count is always positive (starts at 0, only incremented)
                        let delay = Duration::from_millis(1000 * retry_count as u64);
                        warn!("Retrying request after timeout in {:?} (attempt {}/{})", delay, retry_count, max_retries);
                        tokio::time::sleep(delay).await;
                        continue;
                    }
    ...
    fn build_url(&self, endpoint: &str) -> Result<Url> {
        let base_url = Url::parse(&self.config.api_base_url)
            .map_err(|e| JiraError::ConfigError { message: format!("Invalid API base URL: {e}") })?;
        
        base_url.join(endpoint)
            .map_err(|e| JiraError::ConfigError { message: format!("Invalid endpoint URL: {e}") })
    }

    /// Build a request with proper headers and authentication
    fn build_request<U>(&self, method: Method, url: &Url, body: Option<&U>) -> Result<RequestBuilder>
    where
        U: Serialize + ?Sized,
    {
    ...
    pub async fn search_issues(&self, jql: &str, start_at: Option<i32>, max_results: Option<i32>) -> Result<JiraSearchResult> {
    ...
    pub async fn update_issue(&self, issue_key: &str, update_data: &serde_json::Value) -> Result<()> {
        let endpoint = format!("issue/{issue_key}");
    ...
    pub async fn get_comments(&self, issue_key: &str) -> Result<Vec<JiraComment>> {
        ...
        let comments = response.get("comments")
            .and_then(|c| c.as_array())
            .ok_or_else(|| JiraError::ApiError { message: "Invalid comments response format".to_string() })?;
        
        let mut result = Vec::new();
        for comment in comments {
            let comment: JiraComment = serde_json::from_value(comment.clone())
                .map_err(JiraError::SerializationError)?;
            result.push(comment);
        }
    ...
    pub async fn get_transitions(&self, issue_key: &str) -> Result<Vec<JiraTransition>> {
        ...
        let transitions = response.get("transitions")
            .and_then(|t| t.as_array())
            .ok_or_else(|| JiraError::ApiError { message: "Invalid transitions response format".to_string() })?;
    ...
    pub async fn transition_issue(&self, issue_key: &str, transition_id: &str, comment: Option<&str>) -> Result<()> {
        let endpoint = format!("issue/{issue_key}/transitions");
        ...
        if let Some(comment_text) = comment {
            transition_data["update"]["comment"][0]["add"]["body"] = serde_json::Value::String(comment_text.to_string());
        }
